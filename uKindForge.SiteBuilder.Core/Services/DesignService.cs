...
namespace uKindForge.SiteBuilder.Core.Services
{
    public class DesignService(IUmbracoContextAccessor umbracoContextAccessor) : IDesignService
    {
        ...
        public async Task<DesignViewModel> GenerateCssStyle(DesignDetail designDetail)
        {
            ...
            var colorPalette = designDetail.ColorPalette?.ColorPalette ?? new();
            var colorTokens = designDetail.ColorPalette?.ColorToken ?? new();
            var layout = designDetail.LayoutSettings ?? new();

            var cssBuild = new DesignBuilder()
                .BuildTypograpgy(font)
                .BuildColorPalette(colorPalette)
                .BuildHeader(colorTokens)
                .BuildFooter(colorTokens)
                .BuildSections(colorTokens)
                .Build();
